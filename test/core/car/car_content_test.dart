...
void main() {
  ...
  group('CarContent.list', () {
    ...
    test('- appends appears-on albums for an artist once', () async {
      final env = CarTestEnv();
      final content = CarContent(env.container);
      when(() => env.client.getAlbums(any())).thenAnswer((invocation) async {
        ...
        if (query.appearsOnArtistId != null) {
          return LibraryPage(items: [album('own'), album('guest')]);
        }
        return LibraryPage(items: [album('own')]);
      });
      ...
      expect(queries.where((q) => q.appearsOnArtistId == 'ar1'), hasLength(1));
      expect(queries.where((q) => q.artistIds.contains('ar1')), hasLength(2));
    });
