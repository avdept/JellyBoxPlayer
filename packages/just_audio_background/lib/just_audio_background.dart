...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  Future<void> skipToPrevious() async {
    // LOCAL PATCH (JellyBox): restart the current track if we're more than 3s
    // into it (or there's no earlier track), otherwise skip to the previous
    // track. Matches the in-app prev button (SmartPrevious.smartSeekToPrevious)
    // so lock-screen / media-control "previous" behaves the same everywhere.
    // Upstream just called skipToQueueItem(previousIndex!) unconditionally.
    const restartThreshold = Duration(seconds: 3);
    if (currentPosition > restartThreshold || !hasPrevious) {
