...
# upnp_quirks or genre_playlists to somewhere other than the in-repo stubs.
set -euo pipefail
...
for package in upnp_quirks genre_playlists; do
...
if [[ $status -eq 0 ]]; then
  echo "pubspec.lock is buildable without the private packages"
fi
exit $status
